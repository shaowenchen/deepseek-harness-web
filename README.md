...
| `S3_BUCKET` | 否 | 空 | 设置后把桶同步到 `/root`（Node SDK 同步，无需 FUSE） |
| `S3_PATH` | 否 | 空 | 桶内子路径 |
...
| `WORKSPACE_DIR` | 否 | `default` | 首次启动时在 `/root` 下预建的默认工作区目录名（未配 S3 时生效），让 web UI 的目录选择器有默认项 |
...
设了 `S3_BUCKET` 时，容器启动用 Node 同步守护进程（`@aws-sdk/client-s3`）把桶内容拉取到 `/root`（容器 root 用户家目录，即 dsh 的工作区），之后**监听 `/root` 变化**，有改动才触发双向同步（本地变更上传、远端新增下载、本地删除回传），退出时做最终回传。**无需 FUSE**，Railway 等任何容器平台都能跑。连接参数与 storage-console 相同，对金山云 KS3 等已验证可用。
...
|---|---|
| `./data` | `/dsh` |
| `./workspace` | `/root`（容器工作区/家目录，未配 S3 时）；配了 S3 时由 Node SDK 同步的本地目录 |
